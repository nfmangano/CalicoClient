package calico.plugins.analysis.iconsets;
...
public class CalicoIconManager
{
	...
	public static Logger logger = Logger.getLogger(calico.plugins.analysis.iconsets.CalicoIconManager.class.getName());
	...
	public static void setIconTheme(Class<?> clazz, String name)
	{
		...
		try
		{
			iconTheme.load( clazz.getResourceAsStream("/calico/iconsets/"+iconThemeName+"/analysis-icon-theme.properties")  );
	...
	public static Image getIconImage(String iconPath)
	{
		try
		{
			...
			Image icon = toolkit.getImage(new URL(calico.plugins.analysis.iconsets.CalicoIconManager.getIcon(iconPath)));
